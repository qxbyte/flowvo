...
public class PromptsService {
    ...
    // PixelChat相关提示词
    @Value("${prompts.pixel-chat.system:你是一个友善、有帮助的AI助手。当用户上传文件时，请仔细分析文件内容并提供有用的见解。对于图片文件，请描述你能从文件名和上下文中推断的内容。对于文档文件，请根据提供的文本内容进行分析和回答。}")
    private String pixelChatSystemPrompt;

    @Value("${prompts.pixel-chat.file-assistant:你是一个专业的文件分析助手。请仔细分析用户提供的文件内容，根据文件类型提供相应的帮助：对于图片文件：描述图片内容、识别文本、分析场景等；对于文档文件：总结内容、提取关键信息、回答相关问题；对于代码文件：分析代码逻辑、识别问题、提供优化建议}")
    private String pixelChatFileAssistantPrompt;
    ...
    // 图像识别相关提示词
    @Value("${prompts.vision.default:请分析这张图片的内容，描述你所看到的详细信息。}")
    private String visionDefaultPrompt;

    @Value("${prompts.vision.with-question:请根据用户的问题分析这张图片：{question}}")
    private String visionWithQuestionPrompt;
    ...
    // 快捷方法：获取图像问答提示词
    public String getVisionWithQuestionPrompt(String question) {
        return formatPrompt(visionWithQuestionPrompt, "question", question);
    }
} 